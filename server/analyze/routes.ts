...
import { z } from 'zod'
import { createAuditJob, getAuditJob, getAuditResult, startAuditRun } from '../audit/store.js'
...
const usernameSchema = z
  ...
  .min(1)
  .max(39)
  .regex(/^[A-Za-z0-9](?:[A-Za-z0-9]|-(?=[A-Za-z0-9])){0,38}$/u, 'Invalid GitHub username')
  ...
  while (Date.now() - startedAt < timeoutMs) {
    ...
    if (current.status === 'failed') {
      return res.status(500).json({
        error: {
          code: 'ANALYZE_FAILED',
          message: current.message ?? 'Analysis failed',
        },
      })
    }
    await sleep(500)
